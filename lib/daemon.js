...
var valueOrDefault = function(config, prop, fallback) {
  return prop in config ? config[prop] : fallback;
};
...
var daemon = function(config) {
  ...
  Object.defineProperties(this, {
    ...
    stopparentfirst: {
      ...
      configurable: false,
      value:
        config.stopparentfirst instanceof Boolean
    ...
    stoptimeout: {
      ...
      writable: false,
      configurable: false,
      value: valueOrDefault(config, "stoptimeout", 30)
    },
    ...
    nodeOptions: {
      ...
      writable: false,
      configurable: false,
      value: valueOrDefault(config, "nodeOptions", "--harmony")
    },
    ...
    maxRestarts: {
      ...
      writable: false,
      configurable: false,
      value: valueOrDefault(config, "maxRestarts", 3)
    },
    ...
    abortOnError: {
      ...
      configurable: false,
      value:
        config.abortOnError instanceof Boolean ? config.abortOnError : false
    },
    ...
    log: {
      enumerable: false,
      get: function() {
        if (this._eventlog !== null) return this._eventlog;
        if (this.name == null) throw "No name was specified for the service";
    ...
    workingdirectory: {
      ...
      writable: true,
      configurable: false,
      value: valueOrDefault(config, "workingdirectory", process.cwd())
    },
    ...
    script: {
      ...
      value:
        config.script !== undefined
          ? require("path").resolve(config.script)
          : null
    },
    ...
    execPath: {
      ...
      configurable: false,
      value:
        config.execPath !== undefined
          ? require("path").resolve(config.execPath)
          : null
    },
      ...
      value: function(dir) {
        ...
        if (this.exists) {
          ...
          if (!fs.existsSync(path.join(this.root, this.id + ".xml"))) {
            this.log.warn(
              "The primary configuration file is missing or cannot be found (" +
                path.join(this.root, this.id + ".xml") +
                ")"
            );
            missing = true;
          }
          ...
          this.log.warn(
            "The process cannot be installed again because it already exists."
          );
        ...
        fs.exists(dir, function(exists) {
          ...
          fs.writeFile(path.resolve(dir, me.id + ".xml"), me._xml, function() {
            ...
            winsw.createExe(me.id, dir, function() {
              me.execute(
                '"' + path.resolve(dir, me._exe) + '" install',
                function() {
                  sleep(2);
                  me.emit("install");
                }
    ...
    uninstall: {
      ...
      value: function(waitTime) {
        ...
        var uninstaller = function() {
          ...
          me.execute(
            '"' + path.resolve(me.root, me._exe) + '" uninstall',
    ...
    start: {
      ...
      value: function() {
        ...
        this.execute('NET START "' + me._exe + '"', function(
          err,
          stdout,
          stderr
        ) {
          if (err) {
            if (err.code == 2) {
              if (
                err.message.indexOf("already been started") >= 0 &&
                err.message.indexOf("service name is invalid") < 0
              ) {
                me.log.warn(
                  "An attempt to start the service failed because the service is already running. The process should be stopped before starting, or the restart method should be used."
                );
                me.emit("error", err);
                return;
      ...
      value: function() {
        ...
        me.execute('NET STOP "' + me._exe + '"', function(err, stdout, stderr) {
          if (err) {
            if (err.code == 2) {
              me.log.warn(
                "An attempt to stop the service failed because the service is/was not running."
              );
    ...
    exists: {
      ...
      get: function() {
        ...
        return (
          fs.existsSync(path.join(this.directory(), this.id + ".exe")) &&
          fs.existsSync(path.join(this.directory(), this.id + ".xml"))
        );
      }
    },
    ...
    checkPermError: {
      ...
      value: function(error) {
        if (
          error.message.indexOf("Administrator access") >= 0 ||
          error.message.indexOf("Access is denied") >= 0
        ) {
          try {
